...
//I have replaced each T with cln::cl_F
...
public:
  ...
  void identity(cln::float_format_t prec) {
    for (int i = 0; i < n; i++)
      for (int j = 0; j < m; j++)
        elem(i, j) = ((i == j) ? cln::cl_float(1.0) : cln::cl_float(0.0));
  }

  void zero(cln::float_format_t prec) {
    for (int i = 0; i < n; i++)
      for (int j = 0; j < m; j++)
        elem(i, j) = cln::cl_float(0.0);
  }
...
template <class T>
class matrixpslq : public matrix_base<T> {
protected:
  matrixpslq (matrixpslq<T> &a) { }
public:
  matrixpslq(int n0, int m0 = 1) : matrix_base<T>(n0, m0) {
